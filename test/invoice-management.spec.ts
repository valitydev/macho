...
describe('Invoice Management', () => {
    ...
    describe('Create and search invoice', () => {
        ...
        it('should successfully create invoice', async () => {
            const invoiceParams = simpleInvoiceParams(partyID, liveShopID);
        ...
        it('should return logic error invoice have empty cart', async () => {
            const error =
                await invoiceActions.createSimpleInvoice(partyID, liveShopID, 10000, {cart: []})
        ...
        it('should return logic error wrong shopId', async () => {
            const error =
                await invoiceActions.createSimpleInvoice(partyID, 'SHOULDBENOSUCHSHOP')
                    .should.eventually.be.rejectedWith(AxiosError);
            error.response.status.should.be.eq(400);
        ...
        it('should be cancelled if dueDate in the past', async () => {
            ...
            const result = await invoiceActions.createSimpleInvoice(partyID, liveShopID, 10000, {dueDate: past});
    ...
    //         const invoice1 = await invoiceActions.createSimpleInvoice(partyID, liveShopID);
    //         const invoice2 = await invoiceActions.createSimpleInvoice(partyID, liveShopID);
    ...
    //         };
    //         while (tries > 0) {
    //             let promise = invoiceActions.createSimpleInvoice(partyID, liveShopID, params.amount, params);
    ...
    //     const invoiceAndToken = await invoiceActions.createSimpleInvoice(partyID, liveShopID);
    ...
    //     it('should successfully create invoice', async () => {
    //         await invoiceActions.createSimpleInvoice(partyID, liveShopID, 4100000);
    //     });
